...
const ImageBoxControls = () => {
  ...
  return (
    <div className="w-auto bg-zinc-800 rounded-b-lg flex gap-4 items-end justify-center px-10 py-5 absolute top-0 left-1/2 -translate-x-1/2 z-10">
      {/* select theme */}
      <div className="w-fit">
      ...
      <div className="w-fit">
        <span className="options-label">Brand Tag</span>
        <ChnageBrandTag />
      </div>
      <div className="w-fit">
        <span className="options-label">Tag Position</span>
...
function ChnageBrandTag() {
  const { ToggleShowBrandTag, showBrandTag } = useCodeImage();
  return (
    <div className="checkbox-wrapper-51">
      <input
        type="checkbox"
        id="cbx-1"
        className="invisible hidden"
        checked={showBrandTag}
        onChange={(e) => {
          ToggleShowBrandTag(e.target.checked);
        }}
      />
      <label
        htmlFor="cbx-1"
...
function SelectBrandTagPosition() {
  const { brandTabPosition, changeBrandTagPosition, showBrandTag } =
    useCodeImage();
  return (
    <Select onValueChange={changeBrandTagPosition} value={brandTabPosition}>
      <SelectTrigger
        className="w-fit h-fit py-1 px-2 rounded-lg bg-zinc-900 border-zinc-700"
        disabled={!showBrandTag}
      >
        <SelectValue />
      </SelectTrigger>
      <SelectContent className="bg-zinc-900">
